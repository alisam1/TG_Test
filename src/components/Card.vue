<template>
        <div class="card">
            <img v-bind:src="item.image.url" alt="" />
            <p class="card__code" v-bind:class="{ 'red-label': item.code == 'L422WH' ||  item.code =='L423WH'}">{{item.code}}</p>
            <h3 class="card__title">{{item.name}}</h3>
            <p class="card__price card__price--old" v-if="item.price.old_price">{{ formatPrice(item.price.old_price) }}₽</p>
            <p class="card__price">{{ formatPrice(item.price.current_price) }}₽</p>
            <button class="card__basket" @click="$event.target.classList.toggle('isShop')"></button>
            <button class="card__favorite" @click="$event.target.classList.toggle('isFavorite')">
              <svg width="20" height="19" viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M14.0979 1.44916e-05C12.795 0 11.9551 0.155217 10.9924 0.626465C10.6359 0.800937 10.3015 1.01105 9.99095 1.25618C9.69235 1.0248 9.37131 0.824326 9.0296 0.655321C8.04816 0.169898 7.16523 0 5.90907 0C2.54477 0 0.0999756 2.78754 0.0999756 6.40789C0.0999756 9.14198 1.62442 11.7831 4.46393 14.3369C5.95438 15.6774 7.85737 17.004 9.22022 17.7097L9.99998 18.1135L10.7797 17.7097C12.1426 17.004 14.0456 15.6774 15.536 14.3369C18.3755 11.7831 19.9 9.14198 19.9 6.40789C19.9 2.82586 17.4321 0.0138225 14.0979 1.44916e-05ZM18.1 6.40793C18.1 8.54995 16.8276 10.7544 14.3324 12.9986C12.972 14.2221 11.2231 15.4449 10 16.0864C8.77695 15.4449 7.02806 14.2221 5.66767 12.9986C3.17246 10.7544 1.90002 8.54995 1.90002 6.40793C1.90002 3.73298 3.5953 1.80004 5.90911 1.80004C6.90787 1.80004 7.52333 1.91847 8.23164 2.2688C8.65002 2.47573 9.01981 2.74897 9.3399 3.09043L10.0021 3.79687L10.6583 3.08483C10.9856 2.72969 11.3608 2.45023 11.7837 2.24322C12.4713 1.90668 13.0483 1.80004 14.0942 1.80004C16.3794 1.80952 18.1 3.7701 18.1 6.40793Z"/>
              </svg>
            </button>
        </div>
</template>

<script>
export default {
   props: ['item'],

   methods: {
       formatPrice(value) {
        let val = (value/1).toFixed(0)
        return val;
    },
  }
}
</script>

<style lang="scss">
     .card {
        width: 100%;
        height: 100%;
        padding-left: 1.2rem;
        padding-right: 1.2rem;
        padding-bottom: 0.9rem;
        box-sizing: border-box;
        position: relative;

        img {
          width: 90%;
          margin: 0 auto;
        }

        &__code {
          height: 1.8rem;
          margin: 0 0 .6rem 0;
          padding: 0;
          font-size: 1rem;
          line-height: 140%;
          display: flex;
          align-items: center;
          letter-spacing: 0.02em;
          color: #888888;

          &.red-label {
              &::before {
                content: "Скидка";
                position: absolute;
                width: 8.1rem;
                height: 2.4rem;
                background: #EB5757;
                top: 0;
                left: 0;
                font-style: normal;
                font-weight: 500;
                font-size: 1.4rem;
                line-height: 130%;
                letter-spacing: 0.01em;
                color: #fff;
                display: flex;
                align-items: center;
                justify-content: center;
              }
            }
        }

        &__title {
          margin: 0 0 .9rem 0;
          padding: 0;
          font-family: 'SF_Medium', sans-serif;
          font-style: normal;
          font-weight: 500;
          font-size: 1.6rem;
          line-height: 140%;
          letter-spacing: 0.02em;
          color: #000000;
        }

        &__price {
          display: inline;
          margin: 0;
          padding: 0;
          font-style: normal;
          font-weight: normal;
          font-size: 1.6rem;
          line-height: 140%;
          letter-spacing: 0.02em;
          color: #000000;

          &--old {
            font-style: normal;
            font-weight: normal;
            font-size: 1.6rem;
            line-height: 140%;
            letter-spacing: 0.02em;
            text-decoration-line: line-through;
            color: #888888;
            margin: 0 .9rem 0 0;
          }
        }

        &__basket {
            border: none;
            background-color: transparent;
            background-image: url('../assets/basket_icon.svg');
            background-repeat: no-repeat;
            background-position: center;
            width: 2rem;
            height: 2rem;
            position: absolute;
            right: 6rem;
            bottom: 1.5rem;
            cursor: pointer;

            &.isShop {
                background-image: url('../assets/check_icon.svg');
            }
        }

        &__favorite {
            border: none;
            background: transparent;
            cursor: pointer; 
            position: absolute;
            right: 1.2rem;
            bottom: 1.2rem;

            svg {
                fill:black;
            }
            svg.isFavorite {
                fill: #EB5757;
            }
        }
    }
</style>